...
export default {
	...
	async execute({ interaction }) {
		try {
			...
			const existing = await client.db.leave.findFirst({
				where: {
			...
			if (existing) {
				await interaction.reply({
					content: "The leave system is already set up! Please remove it and try again!",
					flags: MessageFlags.Ephemeral,
				});
				return;
			}

			await client.db.leave.create({
				data: {
					channelId: channel!.id,
					message: message!,
				},
			...
			const embed = new EmbedBuilder().setTitle("Leave System Setup").setColor(EmbedColor.green);
